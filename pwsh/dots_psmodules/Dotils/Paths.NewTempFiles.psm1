...
Join-String -inp $PSCommandPath -f 'to wip/implement/finish module: 2023-11-21 {0}' | write-warning

# 'DateNamedFormatCompleter !! Syntax error or else color is enabled'
#     | write-host -bg 'orange'

# using namespace System.Collections


# { $a = 10 } | Get-ScriptBlockDisassembly | sc (join-path 'temp:\

# 'wip: finish module', $PSCommandPath -join ', ' | Write-warning
# write-warning | 'feedback provider?: IFeedbackProvider'



function Tempy.New-TempPathPrefix {
    <#
    .SYNOPSIs
        template for different temp file locations
    #>
    [Alias(
        'Tempy.New-Prefix'
    )]
    ...
    param(
        [ValidateSet(
            ...
            'Temp', 'Temp:', 'Tmp',
            'Win.Temp',
            'H:/data/2023/pwsh/notebooks/Pwsh',
            'h:/data/2023',
            'H:/data/2023/BI/git/ninMonkQuery-examples/forumQuestions',
            'H:/data/2023/BI/PowerQuery',
            'H:/data/2023/BI/excel ⁞ sketch',
            'H:/data/2023/BI/excel ⁞ sketch/excel ⁞ sketch ▸ 2023-11',
            'H:/data/2023/web.js/web.js - sketch 2023-11',
            'H:/data/2023/web.js',
            'H:/data/2023/workspaces',
            'H:/data/2023',
            'H:/data/2023/dump.buffer.offline',
            'H:/data/2023/art/non.sense/2023.02',
            'H:/data/2024'
    ...
    [string]$pathAsStr = switch($TempKind) {
        { $_ -match [regex]::Escape('h:/data/2023') } {
            $_
        }
        default {
            "UnhandledKind: '$TempKind', falling back to 'temp:\'"
                | Write-Warning
            'temp:\'
        }
    }
    if( $AsItem ) {
        $PathAsStr | Join-String -f 'trying: {0}' | Write-Verbose
        return Get-Item $PathAsStr
    }
    $PathAsStr
}
...
class TempyNamedDateTemplate {
    ...
    [CompletionResult] AsCompletionResult () {
        ...
        if( [string]::IsNullOrEmpty( $this.CompletionName) ) {
            ...
            | WriteJsonLog -Text 'ðŸ”´ [CompletionResult]::AsCompletionResult â–¸ ListItem cannot be null!'
...
[hashtable]$script:Colors = [ordered]@{
    BlueGray = '#5f7a8b'
    DimGray = '#819696'
    DimBlue = '#405e7c'
    Fg = '#d4d4d4'
...
if($false -and 'enable debug' ) {
    ...
    @( ( $t )?.AsCompletionResult() )
        | WriteJsonLog -Text '[CompletionResult] top level test ðŸ§ª'
...
function Tempy.Datetime.Format {
    <#
    .SYNOPSIS
        Sugar to format dates that are piped using the same format string

    .EXAMPLE
        Get-Date | Tempy.Datetime.Format -FormatString 'o'
        get-date | Tempy.Datetime.Format -FormatString "MMMM d"
    .EXAMPLE
        Get-Date | Tempy.Datetime.Format -FormatString ( Datetime.Named -DateFormat M/d/yyyy)
        DateTime.Now | Tempy.Datetime.Format -FormatString ( Datetime.Named -DateFormat M/d/yyyy)
    ...
    process {
        $InputObject | %{
            ...
            try {
                ...
                # 'Error.ðŸ’¥ :: Tempy.Datetime.Format. [ Fstr: {0}, Cult: {1}' -f @(
                #     $FormatString ?? ''
                #     $CultInfo ?? ''
                # )
                # | write-warning
...
function Datetime.NamedFormatStr {
    ...
    .EXAMPLE
        Dotils\Dotils.CompareCompletions -Prompt 'Get-Date | Tempy.Datetime.Format -FormatString ' -Verbose -ColumnOrSubstring 'Get-Date | Tempy.Datetime.Format -FormatString '
    .LINK
...
Export-ModuleMember -Function @(
    ...
    'Tempy.Datetime.Format'
    'Tempy.New-TempPathPrefix' # 'Tempy.New-TempPathPrefix' = { 'Tempy.New-Prefix' }
    'Tempy.*'
    'Tmpy.*'
    # 'WriteJsonLog'
 ) -alias @(
    # '.Log'
    'Tempy.New-Prefix' # 'Tempy.New-TempPathPrefix' = { 'Tempy.New-Prefix' }
    'Try.Named.Fstr*'
    'Datetime.Named*'
    'Tempy.*'
    'Tmpy.*'
